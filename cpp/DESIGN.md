...
**히스토그램 칸(bin) 배치 — "범위 밖"과 "깨진 값"을 따로 센다.** 피처마다 정상 구간을
N개로 나눈 경계(`bin_edges`)가 있다. 우리는 칸을 **N+3개**로 만든다 (ABI v2).

```
   bin 0        bin 1..N            bin N+1      bin N+2
 ┌────────┬──────┬──────┬──────┬──────────┬──────────┐
 │underflow│      │ ...  │      │ overflow │   NaN    │
 └────────┴──────┴──────┴──────┴──────────┴──────────┘
   경계보다 작음   정상 구간들      경계보다 큼   깨진 값
```

경계를 벗어난 값을 첫/끝 칸에 욱여넣지(clamp) 않고 **전용 underflow/overflow 칸에 따로
센다** — "값이 학습 때 보던 범위를 벗어났다"는 것 자체가 드리프트 신호다. **NaN도 버리지
않고 전용 칸에 센다** — NaN 비율 급증은 전처리 파이프라인 고장의 제1 신호인데, 조용히
버리면 히스토그램이 멀쩡해 보인다(§14 보강 라운드에서 추가). 단, 드리프트 판정(PSI 등)은
NaN 칸을 **제외한 분포만** 본다 — 품질 문제와 드리프트를 섞지 않기 위해서다.
...
2. 각 feature 값 x 마다:
     - NaN이면 전용 NaN 칸에 +1 (품질 신호로 집계, v2)
     - 아니면 x가 어느 칸인지 이진탐색으로 계산 (underflow/overflow 포함)
     - 그 칸 카운터를 원자적으로 +1
3. 표본 수 +1, inflight-- (쓰기 끝 표시)
4. (1/N 확률) 입력 벡터를 샘플 링에 복사 — 재학습 재료 보존 (v2, §14)
```
...
## 14. 보강 라운드 — 검토(REVIEW.md) 후 추가된 것들

Phase 1~3 완료 후 자체 검토([REVIEW.md](REVIEW.md))에서 찾은 공백 5건을 보강했다.
한 줄씩 요약하면:

1. **탭이 스스로 살아난다 (P1).** worker가 늦게 뜨거나 번들 갱신으로 공유메모리가
   재생성되면, 이전엔 서빙 재시작 전까지 감시 공백이었다. 이제 `tap_maintain()`(하우스키핑
   타이머에서 호출) + degraded 상태 자동 재시도(2^20 호출마다 1회 — 건강할 땐 0 비용)로
   자동 복구된다. worker는 판정이 없어도 ~1초마다 heartbeat를 export하므로,
   `rate(driftmon_samples_total)==0`이면 "탭이 조용히 죽었다"를 off-box에서 즉시 본다.
2. **NaN·데이터 품질 계통 (P2, ABI v2).** NaN을 버리지 않고 전용 칸에 센다. 전처리 고장
   (NaN 홍수·상수화)은 **드리프트가 아니라 품질 알람**으로 분리 — 깨진 데이터로 재학습하는
   사고를 원천 차단. detector는 NaN 칸을 제외하고 분포만 본다.
3. **재학습 재료가 남는다 (P3).** 히스토그램으론 원본 복원이 불가능하다 — 탭이 입력 벡터를
   1/N로 샘플링해 공유메모리 링에 보존하고, worker가 **알람 시 CSV로 덤프**한다. 이 파일이
   재학습/fine-tune 파이프라인의 입력이 된다.
4. **알람을 믿을 수 있다 (P4).** severity 경계가 하드코딩에서 번들로 이동했고, 피처별
   디바운스(오르기 1윈도우/내리기 2윈도우 기본)로 임계 근처 플래핑이 안정 알람으로 보고된다.
5. **다모델 탭 + 무엇이 변했는지 (P5).** `TapHandle`로 서빙 프로세스 하나가 여러 모델을
   탭한다. 판정엔 **kind** 라벨이 실린다: `data_quality`(파이프라인 수리 — 재학습 금지) /
   `out_of_range`(재학습 + bin 재산출) / `abrupt`(원인조사 후 전체 재학습) /
   `sustained`(fine-tune 후보) / `distribution`(재학습 후보). off-box 자동화가 이 라벨로
   대응을 라우팅한다.

E2E 데모도 확장됐다: 출력 드리프트(strongest_angle을 모델 출력으로) + kind 분류
(`out_of_range`) + 알람 시 재학습 샘플 덤프까지 크로스-프로세스로 자동 검증한다.
전체 테스트 25개(전 옵션 ON), hot-path 벤치 p50 ≈ 72ns · 힙 할당 0 · 심볼 audit clean 유지.
