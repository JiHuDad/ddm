...
- [ ] **구현:** `json_min` 최소 파서 — `reference.json` 한정 스키마 파싱 + 불변식 검증
      (SPEC §3). 실패 시 false.
- [ ] **테스트:** reference 왕복/검증 — 유효 JSON 로드, 잘못된 스키마(엣지/비율 불일치,
      합≠1, 빈 features) 거부.
- [ ] **구현:** 히스토그램 누적 — `driftmon_observe`로 특징별 버킷 카운트. 범위 밖
      클램프, NaN/Inf 무시 (SPEC §2.4).
- [ ] **구현:** PSI 계산 — `driftmon_compute` 특징별 PSI + max_psi, epsilon-floor 적용.
- [ ] **테스트:** 알려진 분포로 PSI 값 검증 — 동일 분포 ≈ 0; 시프트 분포 > 0.2.
- [ ] **테스트:** 버킷 경계/빈 버킷/0 나눗셈 케이스 (SPEC §2.4).
